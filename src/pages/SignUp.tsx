...
const SignUp = () => {
  ...
  const handleOpenAISignUp = async () => {
    setLoading(true);
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'azure',
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to sign up with OpenAI',
        variant: 'destructive',
      });
      setLoading(false);
    }
      ...
      <Card className="w-full max-w-md p-8 space-y-6 shadow-xl border-2">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">
            Join us to chat with historical figures
          </p>
        </div>
